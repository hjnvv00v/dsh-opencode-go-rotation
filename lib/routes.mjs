...
export function makeRoutes({ store, keys, usage, configuredSources = () => [], setSourceProvider = () => {} }) {
  const state = async () => {
    const config = await store.get()
    const sourceProviders = configuredSources()
    const selected = sourceProviders.find((provider) => provider.id === config.sourceProvider)
    return {
      ...(await keys.publicState()),
      ...config,
      sourceProviders,
      sourceProviderDisplayName: selected?.displayName || config.sourceProvider,
    }
  }
  ...
  return [
    ...
    { kind: 'exact', path: API.config, handler: async (request, response) => {
      ...
      try {
        if (patch.sourceProvider !== undefined) setSourceProvider(patch.sourceProvider.trim())
